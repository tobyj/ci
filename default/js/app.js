...
// AT: Our only change to this file, unfortunately there is no other
//     way to trigger the document ready event and everything is embedded
//     in this fuction, bad architecture Magento!!
$j(document).on('ready pjax:end', function() {
    ...
    // ==============================================
    // Minicart Desktop On Hover
    // ==============================================
    if ( typeof Modernizr !== "undefined" &&
         typeof Modernizr.mq !== "undefined" ) {

        // Show minicart on hover in desktop only
        $j('#minicart').on('mouseenter', function(e) {
            if (Modernizr.mq("screen and (min-width:" + (bp.medium + 1) + "px)")) { 
                $j('#header-cart').addClass('skip-active');
            }
        });
        
        $j('#minicart').on('mouseleave', function(e) {
            if (Modernizr.mq("screen and (min-width:" + (bp.medium + 1) + "px)")) {
        ...
        if ($j('#header-fixed').length) {
            $j('#header-fixed').on('mouseenter', function(e) {
                 if (Modernizr.mq("screen and (min-width:" + (bp.medium + 1)  + "px)")) {
                    $j(this).removeClass('slim').addClass('full');
                 } else {
                    $j(this).removeClass('slim').removeClass('full');
                 }
            });

            $j('#header-fixed').on('mouseleave', function(e) {
                 if (Modernizr.mq("screen and (min-width:" + (bp.medium + 1)  + "px)")) {
                    $j(this).removeClass('full').addClass('slim');
                 } else {
                    $j(this).removeClass('full').removeClass('slim');
                 }
            });
            ...
            var updateFixedHeader = function() {
                ...
                var headerFixed     = $j('#header-fixed');
                var page            = $j('.wrapper .page');
                var headerPosition  = page.offset().top;
               
                if (Modernizr.mq("screen and (max-width:" + bp.medium + "px)")) {
                    headerPosition += $j('#header-nav').height();
                }
                ...
                if ( currentPosition > headerPosition ) {
                    ...
                    if (Modernizr.mq("screen and (min-width:" + (bp.medium + 1)  + "px)")) {
                        headerFixed.addClass('slim');
                    }

                    page.css('padding-top', headerFixed.height()+'px');
                } else {
                    headerFixed.removeClass('fixed').removeClass('full').removeClass('slim');
            ...
            $j(window).scroll( updateFixedHeader );
    ...
    $j(document)
      .off('click', '#header .skip-link')
      .on('click',  '#header .skip-link', function(e) {
        e.preventDefault();
    ...
    $j(document)
      .off('click', '#header-cart .skip-link-close')
      .on('click',  '#header-cart .skip-link-close', function(e) {
    ...
    enquire.register('(max-width: ' + bp.medium + 'px)', {
        ...
        match: function () {
            if (this.toggleElements) this.toggleElements.toggleSingle();
        },
        unmatch: function () {
            if (this.toggleElements) this.toggleElements.toggleSingle({destruct: true});
        }
    });
