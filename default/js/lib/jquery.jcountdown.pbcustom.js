/*
 *    No info on the library origins
 *
 *    Nick Heskes - Jan 2015
 *    - modified some html structure renders
 */
...
$.fn.countdown = function( method /*, options*/ ) {
    ...
    var msPerHr = 3600000,
        ...
        generateTemplateCustom = function( settings ) {
            ...
            if( !hideDays ) {
                var cDay = formatTime(daysLeft, (daysLeft == 1 && settings.daySingularText) ? settings.daySingularText : settings.dayText, true);
                var cDays = cDay.split('');
                var cDayStr = '';
                if(cDays.length==1){
                    cDayStr = '<span class="time-day-unit">0</span>';//pad the 0
                    cDayStr+= '<span class="time-day-unit">'+cDays[0]+'</span>';
                }else{
                    cDayStr = '<span class="time-day-unit">'+cDays[0]+'</span>';
                    cDayStr+= '<span class="time-day-unit">'+cDays[1]+'</span>';
                }
                template = template.replace('%d', cDayStr );
            } else {
                template = template.replace('%d', '');
            }

            if( !hideHours ) {
                var cHour = formatTime(hrsLeft, (hrsLeft == 1 && settings.hourSingularText) ? settings.hourSingularText : settings.hourText, true);
                var cHours = cHour.split('');
                var cHourStr = '';
                if(cHours.length==1){
                    cHourStr = '<span class="time-hour-unit">0</span>';//pad the 0
                    cHourStr+= '<span class="time-hour-unit">'+cHours[0]+'</span>';
                }else{
                    cHourStr = '<span class="time-hour-unit">'+cHours[0]+'</span>';
                    cHourStr+= '<span class="time-hour-unit">'+cHours[1]+'</span>';
                }
                template = template.replace('%h', cHourStr);
            } else {
                template = template.replace('%h', '');
            }
            
            if( !hideMins ) {
                var cMin = formatTime(minsLeft, (minsLeft == 1 && settings.minSingularText) ? settings.minSingularText : settings.minText, true);
                var cMins = cMin.split('');
                var cMinStr = '';
                if(cMins.length==1){
                    cMinStr = '<span class="time-minute-unit">0</span>';//pad the 0
                    cMinStr+= '<span class="time-minute-unit">'+cMins[0]+'</span>';
                }else{
                    cMinStr = '<span class="time-minute-unit">'+cMins[0]+'</span>';
                    cMinStr+= '<span class="time-minute-unit">'+cMins[1]+'</span>';
                }
                template = template.replace('%i', cMinStr);
            } else {
                template = template.replace('%i', '');
            }
        
            var cSec = formatTime(secLeft, (secLeft == 1 && settings.secSingularText) ? settings.secSingularText : settings.secText);
            var cSecs = cSec.split('');
            var cSecStr = '';
            if(cSecs.length==1){
                cSecStr = '<span class="time-second-unit">0</span>';//pad the 0
                cSecStr+= '<span class="time-second-unit">'+cSecs[0]+'</span>';
            }else{
                cSecStr = '<span class="time-second-unit">'+cSecs[0]+'</span>';
                cSecStr+= '<span class="time-second-unit">'+cSecs[1]+'</span>';
            }
            template = template.replace('%s', cSecStr);
        ...
        generateTemplate = function( settings ) {
            ...
            var $parent = $('<div>'),
                ...
                addTime = function( time ) {
                    /**
                     * adds another span onto each time unit
                     */
                    var cTime = time.split('');
                    var cTimeStr = '';
                    if(cTime.length==1)cTimeStr = '<span class="time-value-unit">0</span>';//pad the 0
                    for(var a=0;a<cTime.length;a++){
                        cTimeStr += '<span class="time-value-unit">'+cTime[a]+'</span>';
                    }

                    timeTasks.push(function() {
                        $parent.append( $timeWrapElement.clone().html( cTimeStr + settings.spaceCharacter ) );
                    });                    
                },
